...
**NOTE:**Kubebuilder is a framework for building Kubernetes APIs using custom resource definitions (CRDs) in Go. The "init" command will sets up a boilerplate project to develop the CRDs/Controllers in GO. The "--domain" option will customise and sets a suffix for the CRDs group. The boilerplate project will have contents similar to this:- "[dell@flcm-rhel-rg22 cars-practice-kube]$ ls
cmd  config  Dockerfile  go.mod  go.sum  hack  Makefile  PROJECT  README.md  test".YOU STILL NEED TO CREATE A API USING "create api" COMMAND TO HAVE CRD/CONTROLLER TEMPELATE CODE.
